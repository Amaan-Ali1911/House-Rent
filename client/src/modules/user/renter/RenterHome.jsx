...
const S = {
  ...
  badge: { fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', background: 'rgba(196,98,45,0.1)', color: '#C4622D', padding: '0.2rem 0.6rem', borderRadius: '4px' },
  greeting: { fontSize: '0.875rem', color: '#7A7470' },
  logoutBtn: { background: '#1C1C1E', color: 'white', padding: '0.45rem 1rem', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 500, border: 'none', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", textDecoration: 'none', display: 'inline-flex', alignItems: 'center', transition: 'background 0.2s' },
    ...
    <div style={S.page}>
      <nav style={S.nav}>
        ...
        <div style={S.navRight}>
          <span style={S.badge}>Renter</span>
          <span style={S.greeting}>Hi, {user.userData.name}</span>
          <Link to="/" onClick={handleLogOut} style={S.logoutBtn}
      ...
      <div style={S.content}>
        <div style={S.header}>
          <div style={S.eyebrow}>Renter Portal</div>
          <h1 style={S.title}>Browse Properties</h1>
        </div>

        <div style={S.tabs}>
          {['All Properties', 'My Bookings'].map((label, i) => (
            <button key={i} onClick={() => setValue(i)} style={S.tab(value === i)}>{label}</button>
          ))}
        </div>
