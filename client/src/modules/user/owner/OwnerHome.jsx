...
const tabs = [
  ...
  { name: "My Properties", component: <AllProperties /> },
  { name: "Bookings", component: <AllBookings /> },
];
...
const S = {
  ...
  badge: { fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', background: 'rgba(212,168,83,0.15)', color: '#9A6A00', padding: '0.2rem 0.6rem', borderRadius: '4px' },
...
const OwnerHome = () => {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.content}>
        <div style={S.header}>
          <div style={S.eyebrow}>Owner Portal</div>
          <h1 style={S.title}>Property Dashboard</h1>
        </div>

        <div style={S.tabs}>
          {tabs.map((tab, index) => (
            <button key={index} onClick={() => setActiveTab(index)} style={S.tab(activeTab === index)}>
              {tab.name}
            </button>
          ))}
        </div>

        <div style={S.panel}>{tabs[activeTab].component}</div>
      </div>
    </div>
  );
};
